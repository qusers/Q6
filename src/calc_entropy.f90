...
subroutine MultiplyMatrices(t)
 ...
 ! u=1.6605402      ! Atomic mass constant *10**(-27) from u to Kg
  ...
  c=(euler**2)*kb*(t%Temperature)*u*0.01_prec/(hs**2)        ! 0.01 is Å to m, u is u to kg (hs*hs) to hs**2
 ...
 elseif( coords(m)%Calc_select == 'No_transl') then
  ...
  '         Frequency (cm**(-1))		  S (cal/mol Kelvin)'                                    
   ...
   do i= 1,no_freq       
     ...
     w = q_sqrt(kb*t%Temperature/(u*t%VIZ(k)))    ! Removed -1.0 since i don't have to shift  ! 10**(12) = sqrt(10**(-23, kb)/(10**(-27, u)*10**(-20, Å**2 to m**2))
