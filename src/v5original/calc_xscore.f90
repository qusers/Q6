...
subroutine XReadAtomTypeConversions(filename, chType)
 ...
 integer       :: fp, p,p2, num_heading = 0, column, count = 0,filestat,i,j
 character(len=256) :: buf,buf2
 character(len=12),allocatable  :: tmp(:)  
 logical :: mark
 mark = .false.
 fp = freefile()
 open(unit=fp,file=filename,err=999,action='READ',status='OLD')
  do
    ...
    elseif(trim(buf).eq.'{protein atom name translations}') then
     ...
     do
      ...
      if(filestat.eq.0) then
       ...
       elseif(trim(buf(1:1)).eq.'#') then ! skip comment
        goto 2     
       elseif(buf(1:1).eq.'{'.and.mark) then
        exit
       else
        p = index(buf,'#')
        if(p.ne.0) buf = buf(1:p-1)    ! remove traling comment
    ...
    elseif(trim(buf(1:1)).eq.'!') then
     ...
     if(buf(1:1).eq.'{') then
      ...
      do
       ...
       elseif(index(buf,'[').ne.0.and.b_atomsection.eq.1) then   ! reach end of [atoms]-section
        b_atomsection = 0
        exit
       else
        if(b_atomsection.eq.1) atom_count(count) = atom_count(count) +1 ! if no rules triggered, this is an atom line
       end if
 2     end do
     end if
    end if
     ...
     if(buf(1:1).eq.'{') then      ! begin residue information
      ...
      do
       ...
       elseif(index(buf,'[atoms]').ne.0) then
        b_atomsection = 1          ! set flag
        i = 0          
       elseif(index(buf,'[').ne.0.and.b_atomsection.eq.1) then   ! reached end of [atoms]-section
        b_atomsection = 0
        exit
       else
        if(b_atomsection.eq.1) then
...
subroutine XReadInput()
 ...
 if(chInput.ne.'default') then
  ! Open file
  fp = freefile()
  open(unit=fp,file=chInput,err=999,action='READ',status='OLD')
   do
    read(fp,'(a256)',iostat=filestat) buf
...
integer function Molecule_Aromatic_Ring_Check_6(molecule,atom_path,bond_path)
 ...
 do i = 0,5 -1 
  ...
  if(mark1.eq.1.and.mark2.eq.1) then
   mark=1
   exit
  else
   goto 1 
...
5 end do
 ...
 if(.not.(present(suppress_warnings)).and..not.(atm%iscofactor.ne.0))then
...
subroutine Dot_PushBack(dots,numdot,newdot)
 ...
 if(associated(dots).eqv..false.) then   ! need to init dot array
