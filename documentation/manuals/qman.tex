...
parallel version of  the main dynamics program  \textbf{qdyn} has been
greatly improved  from that of  version 5.0  which was using  an older
Message Passing  Interface (MPI) standard and  one-sided communication
instead of the more portable point to point communication.
...
parallel version of \textbf{qdyn}.
...
\subsubsection{Constant temperature algorithm}
...
factor. Essentially it inserts an extra particle that will interact 
...
for the thermostat particle, $Q$ is the mass of this particles, and  $g$ is 
the total number of degrees of freedom of the system.

Now, if we call $\textbf{p'} = \textbf{p}/s$, we will have

\begin {equation}
 \mathcal{H}(\textbf{p'},\textbf{r}) = \sum\limits_{i=1}^N
\frac{\textbf{p'}_{i}^2}{2m_{i}s^2} + V(\textbf{r}^N),
\end{equation}
...
It has been shown that a single particle Nos\'e-Hoover particle was not able 
to sample properly the canonical ensemble in some cases. The solution to the 
problem was to create additional particles that would interact with each other 
in a chain-like way \cite{Tuckerman:1992}. The new equations of motion become:
...
\begin{align}
 ...
 \frac{dp_{{s}_{1}}}{dt} &= \left(\sum_{i=1}^N \frac{\textbf{p}_{i}^2}{m_{i}}
...
\hline shake\_solute &  Shake constraining of bonds  to hydrogen atoms
in solute bonds on or off. & Optional, default off.\\
