...
\usepackage [small]{caption2} %Set text in Figure captions
...
energy profile are summarised in the last table generated by \textbf{qfep}.
...
It is possible to handle more than two valence bond states in \textbf{qdyn}
and \textbf{qfep}, however sampling and calibration may be a difficult task
...
corresponding to each interval from \textbf{qfep} output data.

Input is an output-file created by \textbf{qfep}.
...
pdb-file by using the program \textbf{qprep}. Root mean
square coordinate deviation is given in {\AA}.

\subsection{Scoring}\label{subsection:scoring} 
Three     scoring    functions     are    implemented     in    QCalc:
...
are avaliable in QCalc when specifying trajectory or restart files:
\begin{itemize}
...
\subsubsection{Using \textbf{qdyn}}
...
Q is not associated with any  particular force field. The force fields
are  defined in  parameter files,  separate from  the program  and the
choice of force field is thus  simply a matter of which parameter file
to use. Any force field could be  used with the program, as long as it
shares the common functional form of eq.  \ref{eq:V_pot}.
...
(\textbf{qdyn}, \textbf{qprep}, \textbf{qfep}, \textbf{qdum} , \textbf{qcalc}).
